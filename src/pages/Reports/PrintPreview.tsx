...
const PrintPreview: React.FC = () => {
  ...
  const getData = () => {
    ...
    if (selectedReportType === 'credit-notes' && reportData) {
      return (reportData as any)['credit-notes']?.data || [];
    }
  ...
  `;
      ...
      const creditNotes = data as any[];
      if (creditNotes.length === 0) {
        toast.error('No credit note data available');
        return;
      }

      const rows = creditNotes.map((item: any) => `
        <tr>
          <td>${item.date ? formatDate(new Date(item.date)) : '-'}</td>
          <td>${item.description || '-'}</td>
          <td style="text-align: right;">${(item.amount || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
          <td style="text-align: right;">${(item.totalExpenses || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
          <td style="text-align: right;">${(item.remainingBalance || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
        </tr>
      `).join('');
      ...
      const printContent = `
        ...
        <html>
          ...
          <body>
            ...
            <p>Range: ${dateRange && dateRange[0] ? formatDate(dateRange[0]) + (dateRange[1] ? ' - ' + formatDate(dateRange[1]) : '') : 'All Time'}</p>
            <table>
              <thead>
                <tr>
                  <th>Date</th>
            ...
            <div class="footer">${(reportConfigs as any)['credit-notes']?.report_footer || 'Generated by AL LIBAS GENERAL TRADING L L C'}</div>
          </body>
        </html>
      `;
      printWindow.document.write(printContent);
      ...
      if (expenses.length === 0) {
        toast.error('No daily expense data available');
        return;
      }
      ...
      const rows = expenses.map((item: any) => `
        <tr>
          ...
          <td>${item.supplierName || item.supplier?.name || '-'}</td>
          <td>${item.type || '-'}</td>
          <td>${item.description || '-'}</td>
      ...
      const printContent = `
        ...
        <html>
          ...
          <body>
            ...
            <table>
              <thead>
                <tr><th>Date</th><th>Supplier</th><th>Type</th><th>Description</th><th>Payment</th><th>Amount</th></tr>
              </thead>
              <tbody>${rows}</tbody>
              <tfoot>
                <tr>
            ...
            <div class="footer">${(reportConfigs as any)['daily-expense']?.report_footer || 'Generated by AL LIBAS GENERAL TRADING L L C'}</div>
          </body>
        </html>
      `;
      printWindow.document.write(printContent);
          ...
          <div className="bg-white rounded-lg shadow-lg border border-gray-200">
            <div className="min-h-[29.7cm] p-8">
              <div className="flex justify-between items-start mb-8 border-b pb-6">
                ...
                <div className="text-right">
                  <h2 className="text-xl font-bold text-blue-800">CREDIT NOTES REPORT</h2>
                  <p className="text-gray-600 mt-1 text-sm">Generated: {formatDate(new Date())}</p>
                </div>
              </div>
                ...
                <table className="w-full border-collapse">
                  <thead>
                    <tr className="bg-blue-50 border-y">
                      ...
                      <th className="py-2 px-4 text-right text-blue-800 font-medium text-sm">Amount</th>
                      <th className="py-2 px-4 text-right text-blue-800 font-medium text-sm">Total Expenses</th>
                      <th className="py-2 px-4 text-right text-blue-800 font-medium text-sm">Remaining Balance</th>
                    </tr>
                  </thead>
                  <tbody>
                    {data.length > 0 ? (
                      (data as any[]).map((item, index) => (
                        <tr key={index} className="border-b">
                          <td className="py-3 px-4 text-sm">{item.date ? formatDate(new Date(item.date)) : '-'}</td>
                          <td className="py-3 px-4 text-sm whitespace-pre-wrap break-words">{item.description || '-'}</td>
                          <td className="py-3 px-4 text-sm text-right font-medium">₹{(item.amount || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
                          <td className="py-3 px-4 text-sm text-right">₹{(item.totalExpenses || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
                          <td className="py-3 px-4 text-sm text-right font-medium">₹{(item.remainingBalance || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
                        </tr>
                      ))
                    ) : (
                      <tr>
                        <td colSpan={4} className="py-8 px-4 text-center text-gray-600">No credit notes found</td>
                      </tr>
                    )}
                  </tbody>
                </table>
              ...
              <div className="mt-12 border-t pt-8">
                <div className="text-center text-gray-500 text-xs">
                  <p>{(reportConfigs as any)['credit-notes']?.report_footer || `Generated on ${formatDate(new Date())} by AL LIBAS GENERAL TRADING L L C`}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-lg border border-gray-200">
            <div className="min-h-[29.7cm] p-8">
              <div className="flex justify-between items-start mb-8 border-b pb-6">
                ...
                <div className="text-right">
                  <h2 className="text-xl font-bold text-blue-800">DAILY EXPENSE REPORT</h2>
                  <p className="text-gray-600 mt-1 text-sm">Generated: {formatDate(new Date())}</p>
                </div>
              </div>
                ...
                <table className="w-full border-collapse">
                  <thead>
                    <tr className="bg-blue-50 border-y">
                      <th className="py-2 px-4 text-left text-blue-800 font-medium text-sm">Date</th>
                      <th className="py-2 px-4 text-left text-blue-800 font-medium text-sm">Supplier</th>
                      <th className="py-2 px-4 text-left text-blue-800 font-medium text-sm">Type</th>
                      <th className="py-2 px-4 text-left text-blue-800 font-medium text-sm">Description</th>
                      <th className="py-2 px-4 text-left text-blue-800 font-medium text-sm">Payment</th>
                      <th className="py-2 px-4 text-right text-blue-800 font-medium text-sm">Amount</th>
                    </tr>
                  </thead>
                  <tbody>
                    {data.length > 0 ? (
                      (data as any[]).map((item, index) => (
                        <tr key={index} className="border-b">
                          <td className="py-3 px-4 text-sm">{formatDate(item.date || item.createdAt || new Date())}</td>
                          <td className="py-3 px-4 text-sm">{item.supplierName || item.supplier?.name || '-'}</td>
                          <td className="py-3 px-4 text-sm capitalize">{item.type || '-'}</td>
                          <td className="py-3 px-4 text-sm">{item.description || '-'}</td>
                          <td className="py-3 px-4 text-sm uppercase">{item.paymentType || '-'}</td>
                          <td className="py-3 px-4 text-sm text-right">{(Number(item.amount) || 0).toFixed(2)}</td>
                        </tr>
                      ))
                    ) : (
                      <tr>
                        <td colSpan={6} className="py-8 px-4 text-center text-gray-600">No daily expenses found</td>
                      </tr>
                    )}
                  </tbody>
                  <tfoot>
                    <tr className="bg-blue-100 font-semibold border-t-2">
                      ...
                      <td className="py-3 px-4 text-right">
                        {(data as any[]).reduce((sum: number, item: any) => sum + (Number(item.amount) || 0), 0).toFixed(2)}
                      </td>
                    </tr>
                  </tfoot>
                </table>
              ...
              <div className="mt-12 border-t pt-8">
                <div className="text-center text-gray-500 text-xs">
                  <p>{(reportConfigs as any)['daily-expense']?.report_footer || `Generated on ${formatDate(new Date())} by AL LIBAS GENERAL TRADING L L C`}</p>
                </div>
              </div>
            </div>
          </div>
