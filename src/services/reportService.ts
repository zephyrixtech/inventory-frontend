...
export const reportService = {
  ...
  async expenses(params: { from?: string; to?: string } = {}) {
    const query = new URLSearchParams();
    if (params.from) query.append('from', params.from);
    if (params.to) query.append('to', params.to);
    const path = `/reports/expenses${query.toString() ? `?${query.toString()}` : ''}`;
    return apiClient.get<ApiResponse<any[]>>(path);
  },

  async creditNotes(params: { from?: string; to?: string } = {}) {
    const query = new URLSearchParams();
    if (params.from) query.append('from', params.from);
    if (params.to) query.append('to', params.to);
    const path = `/reports/credit-notes${query.toString() ? `?${query.toString()}` : ''}`;
    return apiClient.get<ApiResponse<any[]>>(path);
  }
};
